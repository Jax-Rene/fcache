...
- 服务端收到请求后，判断所有目标列在此期间未发生变化，则返回 304 Not Modified，客户端不更新任何数据
- 服务端收到请求后，若发现至少一个列的Etag值发生变化，则返回对应列的数据以及对应的Etag信息

如下图所示，为客户端和服务端的交互泳道图：

![客户端服务端交互图](./img/客户端服务端交互图.png)

该缓存机制参考了浏览器访问的缓存机制。首先通过服务端设置的Max-Age携带到客户端上，让客户端有一定的存活时间，在存活时间内再次访问不会更新缓存。其次，当存活时间超过后，客户端会发起一次HTTP请求，并且携带上对应列数据的ETag值，服务端收到后只要判断是否有变化过，如果都没变化过返回304状态码，客户端也不需要做任何操作，否则有任何列变化，只返回对应列的新数据。这样做，一来可以减少很多不必要的请求，二来减少了很多不必要的数据传输。



#### 文件下载机制 —— Zero Memory Copy

本质上，fcache 就是一个文件下载系统。传统文件下载系统一般实现有两种方式：

- 基于内存的方式：客户端请求缓存，服务端将数据从用户态内存写向网卡。
- 基于文件的方式：客户端请求缓存，服务端将数据从磁盘读入内存，然后再写入网卡。

基于文件的方式会更繁琐，效率也更低。但是在没有请求的时候，数据落地在文件中，不占用堆内存。这避免了很多内存的问题，例如大量缓存可能会造成大量FULL GC最终影响服务的性能，极端情况下，甚至引起OOM等问题。把数据落地在磁盘上，也使得缓存系统的存储能力有了数十倍的提升。那么优化的方向可以转化为如何提升传统文件的下载效率。在这里，我们尝试使用了零拷贝技术。
...
并且假设每行有个隐形的ROW_KEY可以将不同列按序拼接在一起。假如客户端只想要所有的name数据，那么服务端只需要传输name这一列的数据。同时，假如数据产生了变化，那么在交互上，也只需要更新变化过的列。

**由于我们使用了零拷贝技术，在服务端这里无法做数据筛选等行式的操作。**
...
fcache加载一份缓存数据, 14个列,每个列大小在3M左右. 在本机使用Apache Bench进行性能压测.
仅请求一个列:

| 总请求数 | 一次产生的请求数 | 每次耗时均值 (ms) |
| ------ | ---- | --- |
|1|1|35|
|1000|1|18|
|1000|10|93|
...
加载数据这部分, 通常是很灵活多变的. 从简单的ＳＱＬ查询到复杂的查询计算．在设计的时候，深刻的意识到这块的灵活性是重要的. 因此，这一块我们打算把它做成插件化的.

目前，我们支持两种方式:
1. 直接写SQL从MySQL查询数据然后写入缓存文件
2. 自定义复杂Jar包, 程序会调用Jar包代码计算生成最后的缓存文件

对于方式1,可以应付绝大部分的需求,而对于复杂的,非MySQL的数据,我们通过自己编写Jar也可以灵活实现.



### 03| 横向扩容 & 高可用

关于横向扩容以及高可用的方案, 在分布式领域已经有非常多的种类.

总结下来无外乎以下几类:

1. 使用Zookeeper等中间件实现, 如ClickHouse就是使用Zookeeper实现了它的分片以及副本技术
2. 使用无状态的代理层实现请求的路由, 如Codis就是自定义了无状态的Proxy层, 实现按Key到Slot的分片
3. 使用自定义客户端实现, 如 Redis Cluster 就是使用了它的 Smart Client 实现无中间件的高可用横向扩容
4. 集成Spring Cloud 借助 eurke 以及 zuul 实现

...

上面的每种方案都有其对应优缺点, 因此在选择方案时, 我们还是应该选择最适合自己业务的方案.

由于我们的业务中, 目前有使用到 Spring Cloud, 并且缓存数据不算非常巨大, 单机用磁盘可以承载所有数据, 因此我们考虑直接让服务无状态, 并且使用 Spring Cloud来做.
